...
**待驗證**：需重新 build + 部署到 Staging 後，實測「不清空欄位直接輸入預算 → 送出」應成功，以及「刻意輸入超過 1000000 → 送出」應看到紅色錯誤訊息。
