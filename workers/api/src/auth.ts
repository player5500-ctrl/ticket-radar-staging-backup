...
/**
 * `ACCESS_AUD` 允許用逗號分隔多個 Audience tag。
 *
 * 為什麼需要多個：API 改走 Pages 同源 `/api/*` 之後，送到 Worker 的
 * `Cf-Access-Jwt-Assertion` 是 **Pages 網域** 那個 Access Application 簽的，`aud` 與
 * 原本 API 網域的 Application 不同。兩個都列進來，直接呼叫 Worker 網址測試的舊路徑
 * 也還能通。
 */
function allowedAudiences(value: string): string[] {
  return value
    .split(",")
    .map((item) => item.trim())
...
async function resolveAccessUser(
  ...
  const audiences = allowedAudiences(context.env.ACCESS_AUD || "");
  if (!token || !teamDomain || audiences.length === 0) return null;
