...
import { TicketTaskForm } from "../components/TicketTaskForm";
import type { TicketTaskFormValues } from "../components/TicketTaskForm";
...
export function EventDetailPage() {
  ...
  const taskMutation = useMutation({
    mutationFn: (values: TicketTaskFormValues) =>
      api.createTicketTask({ eventId, ...values }),
    onSuccess: async () => {
  ...
  const reminderMutation = useMutation({
    ...
    onSuccess: async (reminder) => {
      // TASK-03：提醒建立後立刻觸發 .ics 下載，使用者才看得到「有事情發生」。
      downloadReminderIcs(reminder);
      await queryClient.invalidateQueries({ queryKey: ["reminders"] });
    },
  });
  /** 只有被點的那個售票階段要顯示 pending／結果，不要整排一起變。 */
  const pendingReminderWindowId = reminderMutation.isPending
                ...
                <li key={window.id}>
                  ...
                  <div className="timeline__content">
                    ...
                    <button
                      ...
                      type="button"
                      disabled={pendingReminderWindowId === window.id}
                      onClick={() => reminderMutation.mutate(window.id)}
                    >
                      {pendingReminderWindowId === window.id
                        ? "建立中…"
                        : "加入行事曆提醒"}
                    </button>
                    {reminderMutation.isSuccess &&
                      reminderMutation.variables === window.id && (
                        <p className="timeline__reminder-status" role="status">
                          已加入提醒並下載 .ics，也可在「任務」頁重新匯出。
                        </p>
                      )}
                    {reminderMutation.isError &&
                      reminderMutation.variables === window.id && (
                        <p className="timeline__reminder-status timeline__reminder-status--error">
                          {resolveApiError(reminderMutation.error, "提醒").message}
                        </p>
                      )}
                  </div>
                </li>
        ...
        <aside className="action-panel" aria-label="活動操作">
          ...
          {favoriteMutation.isError && (
            <p className="action-panel__error" role="alert">
              {resolveApiError(favoriteMutation.error, "收藏").message}
            </p>
          )}
          {taskMutation.isError && !isTaskFormOpen && (
            <p className="action-panel__error" role="alert">
              {resolveApiError(taskMutation.error, "購票任務").message}
            </p>
          )}
