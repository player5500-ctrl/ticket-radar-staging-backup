...
test("可用表單建立購票任務、完成清單並檢視提醒入口", async ({ page }) => {
  await page.goto("/events/event-blue-hour-taipei");
  await page.getByRole("button", { name: "建立購票任務" }).click();

  const dialog = page.getByRole("dialog", { name: "建立購票任務" });
  await expect(dialog).toBeVisible();

  await dialog.getByLabel("每張預算上限（NT$）").fill("4800");
  await dialog.getByLabel("最多購買張數").fill("2");

  await dialog.getByLabel("可接受場次順位").fill("7/12 19:30 台北場");
  await dialog.getByRole("button", { name: "新增" }).first().click();
  await dialog.getByLabel("可接受區域順位").fill("搖滾區");
  await dialog.getByRole("button", { name: "新增" }).last().click();

  await dialog.getByRole("button", { name: "送出任務設定" }).click();

  await expect(page).toHaveURL(/\/tasks$/);
  await expect(page.getByRole("heading", { name: "購票任務" })).toBeVisible();

  // 綁定到這個活動的任務卡，其他活動也可能有任務。
  const taskCard = page.getByRole("article").filter({ hasText: "藍色時刻：澄音專場" });
  await expect(taskCard).toBeVisible();
  await expect(taskCard.getByText("NT$ 4,800")).toBeVisible();
  await expect(taskCard.getByText("搖滾區")).toBeVisible();
  await expect(taskCard.getByLabel("已確認售票平台帳號")).toBeVisible();
});

test("加入行事曆提醒會觸發 .ics 下載並記錄提醒", async ({ page }) => {
  await page.goto("/events/event-blue-hour-taipei");

  const downloadPromise = page.waitForEvent("download");
  await page.getByRole("button", { name: "加入行事曆提醒" }).first().click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe("ticket-radar-reminder.ics");

  await expect(page.getByText("已加入提醒並下載 .ics")).toBeVisible();

  await page.goto("/tasks");
  await expect(page.getByRole("heading", { name: "提醒與行事曆" })).toBeVisible();
  await expect(page.getByRole("button", { name: "匯出 ICS" }).first()).toBeVisible();
});
